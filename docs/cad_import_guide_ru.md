# Импорт результата slice2solid в CAD и инструменты работы с сеткой
...
В папке результата обычно есть:
- `*_s2s_preview_structure.stl` — сетка восстановленной структуры (имя включает параметры, например `..._vox0p25_ds4_sig1_it20_...`).
- `metadata.json` - параметры запуска, матрица, статистика, `voxel_size_mm` (и доп. сведения о сетке, если включён экспорт геометрии).
- (Опционально) `*_s2s_preview_structure_mesh.ply`, `voxel_points.csv`, `cad_import_notes.txt` - пакет для CAD (CAD bundle):
  - PLY как альтернативный формат импорта сетки,
  - облако точек (point cloud) из занятых вокселей,
...
## Типовая последовательность действий (mesh -> repair -> STEP) в стороннем инструменте

1) Импортируйте сетку (mesh):
- файл: `*_s2s_preview_structure.stl` (или `_mesh.ply`)
- единицы: `mm`

2) При необходимости выполните операции исправления:
...
Для решёток/заполнений иногда лучше работать не с сеткой, а с облаком точек (point cloud):
...
- Если включено разрежение (downsample) при построении поверхности (meshing) и в имени есть `dsN`, эффективный шаг сетки примерно `voxel_size_mm * N`.
- Стартовые значения:
  - по точкам/вокселям: примерно `0.5 * voxel_size_mm`
  - по сетке: примерно `0.5 * voxel_size_mm * N` (если `dsN > 1`)
  - если тяжело по памяти/времени: увеличивайте spacing/resolution
  - если теряются тонкие элементы: уменьшайте spacing/resolution

Если включён пакет для CAD (CAD bundle), в `cad_import_notes.txt` уже записан рекомендуемый старт для spacing/resolution.
