...
3. Ориентация и компенсация усадки фиксируются в матрице `STL -> CMB` внутри `*-simulation-data.txt` (Insight build space). На практике STL, лежащий в папке задания `ssys_*`, часто остаётся в исходных координатах (т.е. без применённого placement).
...
### 3.3 Процесс обработки в slice2solid (GUI-режим)

Ниже описан процесс обработки, который выполняется при запуске через графический интерфейс (GUI) и формирует выходные файлы для CAD/CAE-инструментов (в т.ч. ANSYS).

**Вход (что выбирает пользователь в GUI):**
1. Папка задания `ssys_*` (Insight job folder). Внутри: `*-simulation-data.txt`, `toolpathParams.*` и (для CAD-режима) копия `*.stl`.
2. (Опционально) папка результата (override). По умолчанию: `<ssys_*>/slice2solid_out`.

**Этапы обработки:**
1. **Разбор Simulation Data**
   - читается заголовок (slice height, shrink factors, матрица `STL -> CMB` и др.),
   - валидируется матрица (детерминант/масштаб vs shrink factors) и согласованность габаритов (bbox STL, преобразованный в CMB, должен покрывать bbox траекторий в CMB с допуском).
2. **Приведение геометрии к CMB**
   - траектории в Simulation Data уже заданы в CMB,
   - STL геометрия (если используется) преобразуется в CMB по матрице `STL -> CMB`, чтобы всё было в одной системе координат.
...
5. **Построение поверхности (сетка)**
   - из 3D-объёма строится треугольная сетка (marching cubes),
   - (опционально) сглаживание объёма `sigma` перед построением поверхности (meshing),
...
6. **Экспорт выходных файлов**
   ...
   - при включённом “Геометрия” экспортируется STL (и опционально пакет для CAD (CAD bundle): PLY + облако точек (point cloud) + заметки по импорту),
...
- `metadata.json` - метаданные запуска (параметры, матрица STL -> CMB, статистика/оценки).

`CAD/` (явная геометрия инфилла + файлы для CAD и инструментов работы с сеткой):
- `CAD/*_s2s_preview_structure.stl` - сетка "после" (результат обработки; обычно очень большой файл).
...
`CAE/` (для ANSYS Mechanical):
- `CAE/ansys_layers.json`, `CAE/ansys_layers.csv` — таблица слоёв/направлений укладки (для ортотропии/анизотропии).
- `CAE/ansys_mechanical_import_layers.py` - основной скрипт: создаёт Named Selections/Coordinate Systems; по умолчанию применяет STL -> CMB (если модель уже импортирована в CMB - установите `APPLY_STL_TO_CMB = False`).
- `CAE/ansys_mechanical_section_planes.py` — вспомогательный скрипт: Section Plane по высоте (Z в CMB) и опциональный экспорт PNG по слоям.
- `CAE/cae_import_notes.txt` — краткая инструкция по использованию выходных файлов CAE (в т.ч. скриптов Mechanical).

`DIAG/`:
- диагностические файлы Insight (например, распакованный `.sgm`) для валидации/разбора.
...
- (для CAD-режима) копия `*.stl` в папке `ssys_*` (Insight: Save STL copy in job folder)

STL (тот же файл, который импортирован в Insight) является **эталонной геометрией** для всех последующих этапов. Размещение/усадка задаются матрицей STL -> CMB из `*-simulation-data.txt`; в процессе slice2solid все расчёты слоёв и восстановление геометрии ведутся в координатах CMB (Insight).
...
- `STL to CMB transformation matrix` — преобразование координат геометрии STL в систему CMB (Insight build space).
...
- bbox STL, преобразованный в CMB через `STL -> CMB`, должен покрывать bbox траекторий в CMB (с разумным допуском).
...
2. Система координат Insight build space (**CMB**)
3. Система координат CAE (ANSYS) — координаты импортированной геометрии (обычно исходные STL/CAD, но могут быть и CMB)

Эталонная система координат для траекторий и слоёв — **CMB** (как в Insight). Матрица `STL -> CMB` используется, чтобы привести STL-геометрию к CMB (для CAD-режима) или, наоборот, сопоставить слои CMB с импортированной в CAE геометрией (в скриптах Mechanical).
...
В проекте эталонной системой координат для траекторий считается **CMB**. Поэтому при сопоставлении траекторий с геометрией:

- `XYZ` из таблицы читаются как CMB-координаты,
- при необходимости используется `STL -> CMB` (для CAD-режима) или обратное преобразование `CMB -> STL` (для сопоставления с импортированной геометрией в CAE), полученное из матрицы `STL -> CMB`.

Реализация должна валидировать согласованность: bbox STL, преобразованный в CMB, должен покрывать bbox траекторий в CMB (с допуском).
...
- соответствующее исходной внешней геометрии детали (номинальный CAD/STL),
...
Поэтому конвертацию в “настоящее” CAD-тело следует рассматривать как **задачу внешнего CAD или инструмента работы с сеткой**, а задача slice2solid — подготовить максимально корректные и универсальные данные.
...
3. Если инструмент поддерживает: преобразовать сетку (mesh)/implicit/volume в твердое тело (B-Rep).
...
- `*_s2s_preview_structure.stl` (основной) — сеточная модель результата,
- `*_s2s_preview_structure_mesh.ply` (опционально) — альтернативный формат сетки,
...
### 11.2 Mesh Healer (CAD: исправление сетки) (доп. этап после экспорта сетки)
