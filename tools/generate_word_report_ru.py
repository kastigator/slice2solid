...
def build_report(path: Path) -> Path:
    ...
    _p(
        ...
        "твердотельной CAD-модели (формат, пригодный для импорта как «глухое» тело без истории), "
    ...
    _p(
        ...
        "Для CAD-использования часто требуется получить модель в виде импортируемого твердого тела без дерева построения. "
        ...
        "полученной сетки структуры в твердое тело в специализированных CAD/mesh-инструментах, где "
    ...
    _p(
        ...
        "структуры методом marching cubes (с возможностью разрежения (downsample), sigma и сглаживания) для визуализации и "
    ...
    _p(
        ...
        "*_s2s_preview_structure_mesh.ply, voxel_points.csv, cad_import_notes.txt (пакет для CAD (CAD bundle) для удобного импорта); "
    ...
    _p(doc, "Замечания по применению результатов в CAD и ANSYS.", bold=True)
    _p(
        doc,
        "Для CAD-использования целесообразно рассматривать конечную модель как импортированное "
        ...
        "STEP сохраняет универсальность обмена между CAD-системами. "
